...
1.  Make a list of all phones or phonemes, in descending-frequency order, using
    ...
    [`list_phones.py`](../list_phones.py) is available to facilitate this step.
    Running `./list_phones.py ../tsv/<some-TSV-file> > foo.phones` generates
    `foo.phones` that you can edit by the following steps.
...
4.  In [`../src/`](../src), run `./scrape.py --restriction=<your-lang>`.
5.  In [`../src/`](../src) run `./postprocess && ./generate_summary.py`.
