...
def main():
    ...
    try:
        with open(f"{SERB_CROAT_PATH}{phonetic}") as serb_croat_data:
            split_file(SERB_CROAT_PATH, phonetic, serb_croat_data)
        os.remove(f"{SERB_CROAT_PATH}{phonetic}")
    except FileNotFoundError as err: 
        logging.info(
            'No Serbo-Croatian phonetic tsv: %s',
            err,
        )

    try:
        with open(f"{SERB_CROAT_PATH}{phonemic}") as serb_croat_data:
            split_file(SERB_CROAT_PATH, phonemic, serb_croat_data)
        os.remove(f"{SERB_CROAT_PATH}{phonemic}")
    except FileNotFoundError as err:
        logging.info(
            'No Serbo-Croatian phonemic tsv: %s',
            err,
        )
